...
export default function KeyboardShortcutsPanel() {
  ...
  return (
    <>
      ...
      {/* Shortcuts Panel */}
      <div
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl bg-gray-800 border border-gray-700 rounded-xl shadow-2xl z-50 max-h-[90vh] overflow-y-auto"
        role="dialog"
        aria-modal="true"
        aria-label="คีย์ลัด"
      >
        ...
        <header className="flex items-center justify-between gap-4 px-6 py-4 border-b border-gray-700 sticky top-0 bg-gray-800">
          <h1 className="text-lg font-bold text-white">คีย์ลัด</h1>
          <button
            onClick={closeShortcuts}
        ...
        <div className="px-6 py-4 space-y-6">
          {shortcuts.map((section, idx) => (
            <section key={idx}>
              <h2 className="text-sm font-semibold text-white mb-3 uppercase tracking-wide">{section.group}</h2>
              <div className="space-y-2">
                {section.items.map((item, itemIdx) => (
                  <div key={itemIdx} className="flex items-start justify-between gap-4 py-2">
                    {/* Keys */}
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {item.keys.map((key, keyIdx) => (
                        <div key={keyIdx} className="flex items-center gap-1">
                          ...
                          <kbd className="px-2.5 py-1 bg-gray-700 border border-gray-600 rounded text-gray-300 text-xs font-semibold shadow-sm">
                            {key}
                          </kbd>
                        </div>
                      ))}
